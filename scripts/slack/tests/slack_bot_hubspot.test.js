...
function testDealOwnerResolution() {
  ...
  assert.deepStrictEqual(
    ...
    { id: '87811681', name: 'Mercedes Chien', source: 'explicit deal owner' },
  );
  assert.deepStrictEqual(
    resolveDealHubSpotOwner({ company: 'Hash Co', email: 'buyer@hashco.com' }, { id: '91143842', name: 'Jenilee Chen', source: 'from Slack tag' }),
    { id: '91143842', name: 'Jenilee Chen', source: 'requester is deal owner' },
  );
  const hashed = resolveDealHubSpotOwner({ company: 'Hash Co', email: 'buyer@hashco.com' }, { id: TRUEWIND_HUBSPOT.defaultOwnerId, name: TRUEWIND_HUBSPOT.defaultOwnerName, source: 'default' });
  assert.ok(['84547076', '89305622'].includes(hashed.id));
  assert.deepStrictEqual(
    resolveDealHubSpotOwner({ company: 'Hash Co', email: 'buyer@hashco.com' }, { id: TRUEWIND_HUBSPOT.defaultOwnerId, name: TRUEWIND_HUBSPOT.defaultOwnerName, source: 'default' }),
    hashed,
  );
}
...
async function testProspectWorkflowExplicitOwnerOverridesSlackMappingForContactAndDeal() {
  seedStructuredDealWorkflowProperties();
  const calls = [];
  ...
  try {
    const result = await executeTool('hubspot_push_truewind_prospect', {
      ...
      firstname: 'Buyer',
      lastname: 'Person',
      linkedin_url: 'https://www.linkedin.com/in/buyer-person/',
      lead_source: 'Referral',
      owner_name: 'Alex Lee',
      slack_user_id: 'U_TEST',
      check_duplicates: false,
...
async function run() {
  ...
  await testProspectWorkflowExplicitOwnerOverridesSlackMappingForContactAndDeal();
}
