...
Both routes verify Slack's timestamped HMAC signature before acknowledging. In HTTP mode, `SLACK_STATE_DATABASE_URL` is required. The worker initializes PostgreSQL tables for durable pending deal-source requests and interaction jobs, persists a minimal secret-stripped callback before acknowledging Slack, atomically claims jobs, and recovers jobs that were durably queued but never started. If execution started and the worker crashes or encounters an uncertain external-write error, the job moves to `needs_review` and is never replayed automatically. Without the dedicated database, HTTP mode fails closed at startup and interaction callbacks return `503` rather than acknowledging work that could be lost.

Provision a dedicated Railway PostgreSQL service in the same project and bind its private connection URL to `SLACK_STATE_DATABASE_URL` on the public `leads-update` worker. Do not reuse an unrelated project database or expose the connection URL publicly.

Inspect quarantined jobs with `node scripts/slack/slack_state_admin.js list-needs-review` while `SLACK_STATE_DATABASE_URL` is set. Reconcile each job against Slack and HubSpot before taking any manual action; the command intentionally does not provide an automatic replay operation. Completed rows are retained for 30 days, abandoned pending requests for 7 days, and `needs_review` records for 90 days before automatic deletion.

After the durability gate is deployed and the database readback is healthy, stage `SLACK_EVENT_TRANSPORT=http` on the public `leads-update` worker first, verify its startup and signed routes, then disable Socket Mode in Slack and run top-level mention, thread mention, DM, edited-mention, and deal-source-button canaries. Keep `leads-update-bot` on `socket` until the HTTP canaries pass. For rollback, set the bot service back to `socket`, re-enable Socket Mode in Slack, and immediately redeploy the bot while the HTTP worker remains available.
